...
$result = mysqli_query($db,$query) or die('Error, query failed');
        ...
        while ($row = mysqli_fetch_array($result)) {
           ...
           echo '<tr><td>'.$row['fname'].'</td><td>'.$row['cname'].'</td><td>'.$row['generatedate'].'</td><td>'.'<a href="downloads.php?'.'cid='.$row['cid'].'&fid='.$id.'"><input type = "button" class="btn btn-success" value ="download"></a>'.
           '&nbsp;<a href="download.php?download=true" class="btn btn-info">PO Attaintment</a>'; 
